...
(library (ikarus control)
  ...
  (import 
    ...
    (except (ikarus) call/cf call/cc call-with-current-continuation
            dynamic-wind exit list-tail exit-handler))
  ...
  (define exit-handler
    (make-parameter
      (lambda (status) (foreign-call "ikrt_exit" status))
      (lambda (x)
        (if (procedure? x)
            x
            (die 'exit-handler "not a procedure" x)))))
