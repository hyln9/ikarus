...
(define scheme-library-files
  '(
    ...
    ["libcontrol-5.8.ss"     "libcontrol-5.8.s"     "libcontrol"    ]
    ["libintelasm-5.8.ss"    "libintelasm-5.8.s"    "libintelasm"   ]
    ["libcollect-5.3.ss"     "libcollect-5.3.s"     "libcollect"    ]
    ["librecord-5.6.ss"      "librecord-5.6.s"      "librecord"     ]
    ["libcore-5.7.ss"        "libcore-5.7.s"        "libcore"       ]
    ["libio-5.8.ss"          "libio-5.8.s"          "libio"         ]
    ["libwriter-5.7.ss"      "libwriter-5.7.s"      "libwriter"     ]
    ["libtokenizer-5.7.ss"   "libtokenizer-5.7.s"   "libtokenizer"  ]
    ["libexpand-5.8.ss"      "libexpand-5.8.s"      "libexpand"     ]
    ["libinterpret-5.8.ss"   "libinterpret-5.8.s"   "libinterpret"  ]
    ["libcafe-5.8.ss"        "libcafe-5.8.s"        "libcafe"       ]
      ...
      [(constant c) 
       (cond
        [(or (pair? c) (string? c) (vector? c) (symbol? c))
         (intern c)]
        [(or (boolean? c) (fixnum? c) (char? c) (null? c) 
...
#;(begin
  ...
  (trace-define (tail-indirect-cpr-call) (jmp (list 'indirect (mem (fx- disp-closure-code closure-tag) cpr))))
  (trace-define (indirect-cpr-call) (call (list 'indirect (mem (fx- disp-closure-code closure-tag) cpr))))
...
(begin
  ...
  (define (bigint x) (list 'biginteger x))
  ...
  (define (tail-indirect-cpr-call) (jmp (list 'indirect (mem (fx- disp-closure-code closure-tag) cpr))))
  (define (indirect-cpr-call) (call (list 'indirect (mem (fx- disp-closure-code closure-tag) cpr))))
...
;;;      ac))
  ...
  (define (do-value-prim op arg* ac)
    (case op
      ...
      [($fixnum->char)  
       (list* (movl (Simple (car arg*)) eax)
              (shll (int (fx- char-shift fx-shift)) eax)
              (orl (int char-tag) eax)
              ac)]
      [($char->fixnum) 
       (list* (movl (Simple (car arg*)) eax)
              (shrl (int (fx- char-shift fx-shift)) eax)
              ac)]
      [($fxlognot) 
       (list* (movl (Simple (car arg*)) eax)
              (orl (int fx-mask) eax)
...
(define (asm-helper-code)
  (list
    ...
    (list 'public-function
          ...
          (byte-vector (make-vector (fx- 0 (fx+ wordsize disp-multivalue-rp)) 0))
