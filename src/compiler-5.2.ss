...
(define scheme-library-files
  '(["libsymboltable-4.4.ss" "libsymboltable-4.4.s" "libsymboltable"]
    ...
    ["libcollect-5.2.ss" "libcollect-5.2.s" "libcollect"]
...
(define pcb-table
 '(;;; system locations used by the C/Scheme interface
   ...
   [$stack-extent       system       "stack_extent"]
   [$roots              system              "roots"]
...
(define (remove-local-variables x)
  ...
  (define (Tail x si r live)
    (record-case x
      ...
      [(var) 
       (cond
         [(assq x r) => (lambda (x) (make-return (cdr x)))]
         [else (error who "unbound var ~s" x)])]
      [(primref) (make-return x)]
      [(closure code free*) 
       (make-return (make-closure code (simple* free* r)))]
      [(bind lhs* rhs* body)
      ...
      [(primcall op arg*)
       (make-return (make-primcall op (simple* arg* r)))]
      [(forcall op rand*)
       (make-return (do-new-frame op rand* si r 'foreign live))]
      [(funcall op rand*)
...
(define (generate-code x main-name)
  ...
  (define (do-pred-prim op rand* Lt Lf ac)
    (case op
     ...
     [(vector?)    (type-pred vector-mask vector-tag rand* Lt Lf ac)]
  ...
  (define (do-effect-prim op arg* ac)
    ...
    [($set-symbol-value!) 
     (list* (movl (Simple (car arg*)) eax)
            ...
            ac)]
    [($exit)
     (cons (jmp (label "L_scheme_exit")) ac)]
    [(cons void)
     (let f ([arg* arg*])
       (cond
         [(null? arg*) ac]
 ...
 (define (emit-prog prog main-name)
    (list*
      (list 'public-function 
           ...
           (movl (mem (pcb-offset '$scheme-stack) pcr) esp)
           (jmp (label "L_scheme_call"))
           (byte-vector '#(0))
           (int wordsize)
           (label "L_scheme_call")
      ...
      (list 'local-function
            "L_foreign_call"
            0
            (movl fpr (mem (pcb-offset '$stack-extent) pcr))
            (movl apr (mem (pcb-offset '$allocation-pointer) pcr))
            (movl fpr ebx)
            (movl (mem (pcb-offset '$system-stack) pcr) esp)
            (pushl pcr)
            ...
            (movl (mem (pcb-offset '$stack-extent) pcr) fpr)
            (movl (mem (pcb-offset '$allocation-pointer) pcr) apr)
            (ret))
      (list 'local-function
            "L_apply"
...
(define (compile-program-with-entry original-program scheme-entry)
  (let* (;;;
         ...
         [checked (insert-allocation-checks simplified)]
         [no-loc (remove-local-variables checked)]
         [linearized (generate-code no-loc scheme-entry)])
...
(runtime-file 
  (string-join " " 
    (list* "scheme.c" "runtime-5.2.c" "collect-5.2.c" "libtoplevel.s"
