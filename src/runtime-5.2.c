...
int main(int argc, char** argv){
  ...
  fprintf(stderr, "pcb=0x%08x\n", (int)pcb);
  pcb->heap_base = heap;
  pcb->heap_size = (char*) heap_size;
  pcb->allocation_pointer = heap;
  pcb->allocation_redline = heap + heap_size - 2 * pagesize;
  pcb->scheme_stack = stak + stak_size;
#if 0
  fprintf(stderr, "scheme_stack=0x%08x\n", (int)pcb->scheme_stack);
#endif
  scheme_main(pcb);
#if 0
  fprintf(stderr, "%d bytes used\n", pcb->allocation_pointer - heap);
#endif
  deallocate_protected_space(stak, stak_size);
